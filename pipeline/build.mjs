...
// Randstad: ONE feed for the whole of the Netherlands (gtfs.ovapi.nl, 3167
// lines), cut down to the western wing — Amsterdam, Haarlem, Leiden, Den Haag
// and Rotterdam — by a precomputed rectangle, not a radius (scope.mjs →
// data/scope.json). Three modes: the buses of a dozen concessions navy, three
// separate tram networks family red, and the two metros — GVB's 50–54 and
// RET's A–E — in the colours the feed itself ships.
// Usage: node pipeline/build.mjs [--all | lines...] [--tram all|gvb:1,ret:A]
...
// Exact-match repairs for poles a feed names badly. The Dutch national feed
// needs none — it is the best-cased of the family, every stop already written
// the way the pole is.
...
// ONE feed, THREE cfgs.
//
// gtfs.ovapi.nl publishes the whole Dutch national timetable in one bundle —
// every concession in the country. The map is the Randstad's western wing, so
// pipeline/scope.mjs precomputes the allowlist by RECTANGLE (a radius makes no
// sense for a 75 km string of five cities). Route types are the plain GTFS
// ones: 3 bus, 0 tram, 1 metro.
//
// Cut deliberately: the trains (2 — NS names every one of them "Intercity" or
// "Sprinter" and keeps the line number internal, so there is nothing to draw
// and letter; the same call Göteborg's "TÅG" got), the ferries (4 — GVB's
// F1–F22 across the IJ and the Waterbus to Dordrecht; no water graph), the
// 83 "Stopbus/Snelbus ipv trein" rail replacements, the 33 dial-a-ride runs
// (U-Flex, BestelBuzz, HopOn, OV op Maat) and the Amsterdam museum tram.
//
// LINE KEYS: three tram networks and a dozen bus concessions all number from
// 1, so a number used by more than one operator carries that operator's code
// in the KEY (gvb:1, htm:1, ret:1) and prints bare through LBL — the Rybnik
// rule. Nothing prints twice on any street: the networks share no pavement.
const LBL = new Map();
// line key → operator code, so meta can group the panel by operator
...
const S_BUS = new Set(SCOPE.bus), S_TRAM = new Set(SCOPE.tram), S_METRO = new Set(SCOPE.metro);
const KEY = SCOPE.key || {}, OP = SCOPE.op || {};

// no trunk mode: both metros are ordinary urban metros drawn as such by the
// tram cfg's metro treatment, and there is no commuter-rail layer here
const isRailTrunk = () => false;

const lineKey = (sn, r) => {
  const k = KEY[r.route_id] || sn;
  if (!k) return null;
  if (k !== sn) LBL.set(k, sn);   // the operator code never reaches the street
...
const MODES = [{
  ...
  // the western Randstad — 5 × 5 tiles over 4 500 km² of the densest street
  // grid in Europe, cut out of three Geofabrik provinces (pbf-tiles.py)
  osmFiles: Array.from({ length: 25 }, (_, i) => `data/osm/tiles/t${i + 1}.json`),
  color: '#0059a9', colorDark: '#00294f',
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['gvb:15']),
  feeds: [
    { tag: 'nl', dir: 'data/gtfs', routeTypes: ['3'],
      skipRoute: (r) => !S_BUS.has(r.route_id), mapKey: lineKey },
  ],
}];
const tramAll = tramLines.length === 1 && tramLines[0] === 'all';
const metroSel = tramLines.filter((l) => /^(gvb:)?5[0-4]$|^ret:[A-E]$/.test(l));
const tramSel = tramLines.filter((l) => l !== 'all' && !metroSel.includes(l));
if (tramAll || tramSel.length) MODES.push({
  // GVB Amsterdam, HTM Den Haag (RandstadRail 3 and 4 included — they run on
  // former heavy-rail alignment OSM tags light_rail) and RET Rotterdam
  mode: 'tram', label: 'trams', osmFile: 'data/osm/randstad-rail.json',
  graphMode: 'tram', railKeep: new Set(['tram', 'light_rail']),
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : tramSel,
  feeds: [
    { tag: 'nl', dir: 'data/gtfs', routeTypes: ['0'],
      skipRoute: (r) => !S_TRAM.has(r.route_id), mapKey: lineKey },
  ],
});
if (tramAll || metroSel.length) MODES.push({
  // two unrelated metros in one frame: GVB's 50–54 under Amsterdam and RET's
  // A–E, whose line E reaches Den Haag Centraal over the RandstadRail
  // alignment — so light_rail belongs in this graph as well as the trams'
  mode: 'tram', label: 'metro', osmFile: 'data/osm/randstad-rail.json',
  graphMode: 'tram', railKeep: new Set(['subway', 'light_rail']),
  allMetro: true,
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : metroSel,
  feeds: [
    { tag: 'nl', dir: 'data/gtfs', routeTypes: ['1'],
      skipRoute: (r) => !S_METRO.has(r.route_id), mapKey: lineKey },
  ],
});
...
await (await import('./night.mjs')).nightPass(outDir, /^[Nn]\d/, { sort: true });
...
await (await import('./railrows.mjs')).railRowPass(outDir, /^[Nn]\d/, { log });
