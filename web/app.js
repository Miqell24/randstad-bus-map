...
async function init() {
  ...
  map.addControl(new maplibregl.AttributionControl({ compact: true, customAttribution: 'Timetables: OVapi (Dutch national GTFS)' }));
  ...
  // Five cities in one frame means the same number several times over: three
  // tram "1", a dozen bus "2". Each chip prints what its own street signs, so
  // the list is grouped by the concession that runs it — the Rybnik panel.
  const OPS = [
    ['gvb', 'GVB — Amsterdam'], ['htm', 'HTM — Den Haag'], ['ret', 'RET — Rotterdam'],
    ['conn', 'Connexxion'], ['meer', 'MeerPlus — Haarlemmermeer'], ['ebs', 'EBS — Waterland & Haaglanden'],
    ['qbuzz', 'Qbuzz'], ['tdev', 'Transdev'], ['uov', 'U-OV'],
  ...
  async function exportBBox(bbox, btn, doneLabel, opts) {
    ...
    try {
      ...
      const txt = '© OpenStreetMap contributors · OpenFreeMap · Timetables: OVapi (Dutch national GTFS)';
      ...
      a.download = `randstad-transit_${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}_${pad(d.getHours())}${pad(d.getMinutes())}_${out.width}x${out.height}.png`;
  ...
  btnAtlas.addEventListener('click', async () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.download = `randstad-transit-giant_${d.getFullYear()}-${p2(d.getMonth() + 1)}-${p2(d.getDate())}_${Wf}x${Hf}.png`;
      a.href = URL.createObjectURL(blob);
  ...
  // no single core here: the opening view is the whole western wing,
  // Amsterdam to Rotterdam, because that string of five cities IS the subject
  map.jumpTo({ center: [4.620, 52.130], zoom: 8.6 });
}
